...
- **应用功能绑定**：自动读取运行中 App 暴露的菜单功能，搜索后绑定键帽；未暴露的功能可手动录入快捷键。功能绑定每次执行，普通应用绑定仍然切换唤起／隐藏。
...
1. 打开面板 → **编辑** → 点击一个字母、数字或标点键帽。
2. 选择 **应用功能**，搜索并选择目标 App。
3. 在 **自动发现菜单** 中搜索功能名称或菜单路径。App 未运行时可点击 **启动并读取**；已运行时会自动读取，也可刷新。
4. 选择一个菜单功能，点击 **测试** 或 **保存绑定**。执行时先激活目标 App，按完整菜单路径重新查找并调用该菜单项。
5. 功能键帽右下角显示闪电标识，悬停可看到 App 与功能名称。以后按统一前缀加该键，或点击键帽，就执行功能。

**普通应用绑定仍然保持启动／唤起／隐藏切换。** 将绑定类型切回 **应用 · 唤起 / 隐藏** 后重新选择 App，即可恢复该行为。清除绑定的操作不变。

### 没有自动发现的功能，例如微信截图

并非所有 App 的全局快捷键都会暴露到菜单。本机微信 4.1.7.2 可读取菜单功能，但当前菜单没有发现“截图／截屏”；KeyDock 不会猜测或硬编码它的快捷键。

在 **手动添加快捷键** 中：填写“微信截图” → 点击录入按钮 → 按下微信设置中实际配置的截图快捷键。如果该快捷键可在其他应用中使用，选择 **后台触发**；仅在 App 内有效的快捷键选择 **先激活应用，再执行**。

- 录入支持 Control／Option／Command 与常见按键组合（可加 Shift），以及 F1–F12；Esc 取消录入。Fn 组合暂不支持录入。
- **测试会实际执行功能，但不会保存绑定**。面板先收起，等待按键松开后执行；再次呼出面板可继续编辑未保存的草稿。成功后不自动弹回，避免影响截图。测试错误会返回编辑器。
- 后台触发不主动激活已运行的 App；未运行时会请求后台启动。目标 App 可能自行显示启动／登录窗口，或需要先完成登录才能工作。
- “已发送快捷键”仅表示事件已发送，不能证明目标 App 已完成截图等操作，请观察实际效果。
- 菜单处于禁用状态时不能执行。同名完整路径、菜单变化、权限不足或读取不完整会提示错误；不会按旧位置猜测点击其他菜单。
- 读取和执行都需要辅助功能授权。不扫描 App 的私人配置文件，不声称能枚举所有内部或全局快捷键。
...
使用带版本号的 JSON 原子写入，保存按键、应用标识、应用路径和可选功能信息（菜单路径或快捷键）。1.1.0 可读取旧版 v1 配置；首次保存为 v2 前，会将原文件保留为同目录的 `settings.v1-backup.json`。读取配置本身不会改写原文件。应用移动后会尝试按标识重新定位；配置损坏时保留原文件，由用户在设置中选择备份并重置。
...
- 当前 31 项测试通过；已实机验证面板配置与通过键帽显示、隐藏微信。完整实体快捷键、睡眠唤醒、多屏和全屏空间等场景仍需按 [验证记录](docs/QA.md) 回归。实际验证环境为 macOS 27 / Xcode 27 beta，尚未在 macOS 14 设备上运行。
