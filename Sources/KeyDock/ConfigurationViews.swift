    ...
    private func chooseFile() {
        ...
        panel.begin { response in
            ...
            do {
                ...
                model.assign(app, bookmark: bookmark)
            } catch { model.errorMessage = "无法保存应用访问权限：\(error.localizedDescription)" }
        }
    }
}
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 22) {
                    section("快捷键") {
                        settingsRow("统一前缀键", detail: "前缀 + 字母、数字或标点，调用对应 App。") {
                            Picker("统一前缀键", selection: Binding(get: { model.configuration.prefix }, set: model.setPrefix)) {
                                ForEach(Modifier.storeChoices) { Text("\($0.symbol)  \($0.title)").tag($0) }
                            }.labelsHidden().frame(width: 170)
                        }
                        settingsRow("面板呼出键", detail: "所选修饰键 + 空格，打开或关闭键盘面板。") {
                            Picker("面板呼出键", selection: Binding(get: { model.configuration.summonKey }, set: model.setSummonKey)) {
                                ForEach(Modifier.storeChoices) { Text("\($0.symbol)  \($0.title)").tag($0) }
                            }.labelsHidden().frame(width: 170)
                        }
                    ...
                    section("快捷键状态") {
                        ...
                        Text("商店版通过系统注册快捷键，无需辅助功能权限。已被系统或其他应用占用的组合会显示冲突；可更换前缀或呼出键后重试。编辑或聚焦设置时暂时释放快捷键。")
                            .font(.caption).foregroundStyle(.secondary).fixedSize(horizontal: false, vertical: true)
                        HStack {
                            Button("重新检测") { model.monitor.retry() }
                            Button("显示当前 App") { NSWorkspace.shared.activateFileViewerSelecting([Bundle.main.bundleURL]) }
                        }
                        Text("收到注册快捷键事件：\(model.globalEventCount) · 呼出次数：\(model.summonCount)")
                    ...
                    Button("隐私政策") { model.showPrivacyPolicy() }
